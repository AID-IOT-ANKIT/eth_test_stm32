...
void ModbusEthernet::config (uint8_t * mac, IPAddress ip) {
  Ethernet.begin (mac, ip);
  _server.begin();
}

void ModbusEthernet::config (uint8_t * mac, IPAddress ip, IPAddress dns) {
  Ethernet.begin (mac, ip, dns);
  _server.begin();
}

void ModbusEthernet::config (uint8_t * mac, IPAddress ip, IPAddress dns, IPAddress gateway) {
  Ethernet.begin (mac, ip, dns, gateway);
  _server.begin();
}

void ModbusEthernet::config (uint8_t * mac, IPAddress ip, IPAddress dns, IPAddress gateway, IPAddress subnet) {
  Ethernet.begin (mac, ip, dns, gateway, subnet);
  _server.begin();
}

void ModbusEthernet::task() {
